...
<template>
    <Teleport to="body">
        <div v-show="cameraStore.opened" v-if="cameraStore.opened" class="fixed inset-0 z-50 bg-black">
            <!-- Live viewfinder -->
            <video v-show="!cameraStore.previewUrl" ref="videoEl" id="cameraVideo" autoplay playsinline
                class="absolute inset-0 w-full h-full object-contain"></video>

            <!-- Preview after capture -->
            <img v-show="cameraStore.previewUrl" :src="cameraStore.previewUrl ?? undefined"
                class="absolute inset-0 w-full h-full object-contain" />

            <canvas id="cameraCanvas" class="hidden"></canvas>

            <!-- Camera selector -->
            <div v-if="cameraStore.devices.length > 0 && !cameraStore.previewUrl"
                class="absolute top-4 left-0 w-full px-4 flex justify-center gap-2 z-10">
                <select v-model="cameraStore.selectedDeviceId"
                    class="px-4 py-2 h-10 bg-gray-800 text-white rounded-full shadow-lg ring-1 ring-gray-600">
                    <option v-for="device in cameraStore.devices" :key="device.deviceId" :value="device.deviceId">
                        {{
                            device.label ||
                ...
                <button type="button" @click="cameraStore.loadDevices()"
                    ...
                    title="Reload cameras" aria-label="Reload cameras">
                    <RefreshIcon :size="20" />
                </button>
            </div>

            <!-- Shooting controls -->
            <div v-show="!cameraStore.previewUrl"
                ...
                <button type="button" @click="cameraStore.capture()"
                    class="h-16 w-16 bg-white rounded-full shadow-lg border-4 border-gray-300 hover:bg-gray-100 active:scale-95"
                    title="Capture photo"></button>
                <button type="button" @click="cameraStore.close()"
