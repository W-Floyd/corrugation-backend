document.addEventListener("alpine:init", () => {
  ...
  Alpine.store("moveEntityDialog", {
    ...
    open(x) {
      ...
      this.reset();
      this.targetLocation =
        Alpine.store("entities").fullstate.entities[x].location;
      this.opened = true;
    },
        ...
        for (const cid in ret) {
          ...
          if (
            Alpine.store("entities")
              .fullstate.entities[id].name.toLowerCase()
              .includes(searchText.toLowerCase()) ||
            Alpine.store("entities")
              .fullstate.entities[id].description.toLowerCase()
              .includes(searchText.toLowerCase()) ||
            id == searchText.toLowerCase()
          ) {
            if (
              id == searchText ||
              Alpine.store("entities").fullstate.entities[
                id
    ...
    formatOption(x) {
      ...
      while (target != 0) {
        if (
          Alpine.store("entities").fullstate.entities[target].name == null ||
          Alpine.store("entities").fullstate.entities[target].name == ""
        ) {
          tree.push(target);
        } else {
  ...
  Alpine.store("api", {
      ...
      for (key in Alpine.store("newEntityDialog").files) {
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status == 200) {
            ...
            Alpine.store("newEntityDialog").entity.artifacts.push(
              parseInt(response),
            );
          }
        };
    ...
    uploadArtifactsEdit() {
      Alpine.store("editEntityDialog").files = [
        Alpine.store("editEntityDialog").files.reverse()[0],
      ];
  ...
  Alpine.store("entities", {
    ...
    load(matchID, searchText) {
      if (searchText != "") {
        ...
        if (this.filterworld) {
          children = this.listChildLocationsDeep(0).sort((a, b) =>
            sortEntityID(a, b),
          );
        } else {
          children = this.listChildLocationsDeep(this.currentEntity).sort(
            (a, b) => sortEntityID(a, b),
          );
        }
        ...
        for (const cid in children) {
          id = children[cid];
          if (
            this.fullstate.entities[id].name
              .toLowerCase()
              .includes(searchText.toLowerCase()) ||
            (this.searchdescription &&
              this.fullstate.entities[id].description
                .toLowerCase()
                .includes(searchText.toLowerCase())) ||
            id == searchText.toLowerCase()
          ) {
            if (
              ...
              if (
                id == searchText ||
                this.fullstate.entities[id].name.toLowerCase() ==
    ...
    selectImages(x) {
      // Returns the IDs of the artifacts
      if (
        Alpine.store("entities").fullstate.entities[x].artifacts != null &&
        Alpine.store("entities").fullstate.entities[x].artifacts.length > 0
      ) {
        images = [];
